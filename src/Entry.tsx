...
export function Entry({onStart}:{onStart:()=>void}){
 ...
 {step===0?<div className="clan-layout"><div><div className="entry-preview" aria-label="甲賀の完成キューブの見本"><Scene preview/></div><p className="entry-caption">甲賀の六人。ひと回しで、絵がつながる。</p></div><div className="clan-list">{['甲賀','伊賀','風魔','雑賀','根の国'].map(name=><button key={name} aria-pressed={clan===name} onClick={()=>setClan(name)}><strong>{name}</strong><span>{name==='甲賀'?'静止画・アニメで遊べます':'準備中'}</span></button>)}<p className="entry-note">{clan==='甲賀'?'咲耶・シャオラン・ネム・イズナ・ウカ・オト':'このクランの完成キューブは準備中です。左の見本は甲賀です。'}</p><button className="entry-primary" onClick={()=>setStep(1)}>{clan}を選ぶ →</button></div></div>:null}
