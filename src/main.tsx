...
function App(){
 ...
 <div className="game-menu"><span>甲賀 / {difficultyLabel[s.difficulty]} · {s.lastMix}手でスタート</span><button onClick={()=>{setPlaying(false);window.scrollTo(0,0);}}>← 条件を選び直す</button></div>
 ...
 <details className="help"><summary>遊び方・操作のヒント</summary><div className="help-body"><h3>{s.selection?faces[s.selection.face].name+'のマスを選択中':'まずはマスを選択'}</h3><p>PC：マスをクリックして W A S D。<br/>スマホ：マスの少し外側からでも、長押ししてスワイプできます。<br/>表示された矢印ボタンでも回せます。</p><p>画面の余白やキューブをドラッグすると、全体を眺められます。<br/>別の列を回すときは、マスを選び直します。</p><p>{s.mode==='one'?'どの面でも、1面の位置と向きが揃えばクリア。':'位置と向きが6面とも揃うと完成。'}<br/>まずは3手だけ混ぜて、1手ずつ戻してみてください。</p></div></details>
