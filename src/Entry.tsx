...
export function Entry({onStart}:{onStart:()=>void}){
 ...
 {step===0?<div className="clan-layout"><div><div className="entry-preview" aria-label="甲賀の完成キューブの見本"><Scene preview/></div><p className="entry-caption">甲賀の六人。ひと回しで、絵がつながる。</p></div><div className="clan-list">{['甲賀','伊賀','風魔','雑賀','根の国'].map(name=><button key={name} aria-pressed={clan===name} onClick={()=>{setClan(name);setStep(1);}}><strong>{name}</strong><span>{name==='甲賀'?'静止画・アニメで遊べます':'準備中'}</span></button>)}<p className="entry-note">{clan==='甲賀'?'咲耶・シャオラン・ネム・イズナ・ウカ・オト':'このクランの完成キューブは準備中です。左の見本は甲賀です。'}</p></div></div>:null}
 {step===1?<><p className="entry-caption">{clan} / 絵の動きを選ぼう</p><div className="difficulty-list">{([['易','静止した顔の絵を揃えよう。','easy'],['中','6人の表情や髪が動く絵を揃えよう。','medium'],['難','全身と背景が動く絵を揃えよう。','hard']] as const).map(([label,description,tone])=>{const ready=clan==='甲賀'&&tone!=='hard';return <button className={tone} key={label} disabled={!ready} onClick={()=>{if(tone!=='hard'){game.setDifficulty(tone);setStep(2);}}}><b>{label}</b><span>{description}<small>{ready?'挑戦する →':'準備中'}</small></span></button>;})}</div><p className="entry-note">完成したステージは、クリア条件なしで自由に選べます。</p></>:null}
 {step===2?<form className="entry-count" onSubmit={e=>{e.preventDefault();if(valid&&game.start(Number(count)))onStart();}}><p className="entry-caption">{clan} / {game.difficulty==='medium'?'中・アニメ':'易・静止画'}</p><div className="count-presets">{[3,6,12,24].map(n=><button type="button" aria-pressed={Number(count)===n} key={n} onClick={()=>setCount(String(n))}>{n}手</button>)}</div><label htmlFor="entry-count">混ぜる回数 <input id="entry-count" type="number" min="3" max="30" required step="1" value={count} onChange={e=>setCount(e.target.value)} aria-describedby="count-note"/> 手</label><p id="count-note" className="entry-note">3〜30手。混ぜる回数です。クリアまでの手数ではありません。</p><fieldset><legend>クリア条件</legend>{(['one','six'] as const).map(mode=><label key={mode}><input type="radio" name="goal" checked={game.mode===mode} onChange={()=>game.setMode(mode)}/>{mode==='one'?'どれか1面':'6面すべて'}</label>)}</fieldset><button className="entry-primary" disabled={!valid}>挑戦する →</button><p className="entry-note">混ざった状態から、すぐに遊べます。</p><p role="status">{game.notice.includes('指定')||game.notice.includes('作れません')?game.notice:''}</p></form>:null}
 </section>;
}
